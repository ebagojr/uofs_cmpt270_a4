...
public class HospitalSystemA4Q1
{
    ...
    public void displayEmptyBeds()
    {
        ...
        for (int i = ward.getMinBedLabel(); i <= ward.getMaxBedLabel(); i++)
            if (! ward.isOccupied(i))
                bedList.addLast(i);
        String result = "\nWard " + ward.getName() + " with the following free beds: ";
        for (int i = ward.getMinBedLabel(); i <= ward.getMaxBedLabel(); i++)
            if (! ward.isOccupied(i))
                result = result + "\nbed " + i + ": ";
    ...
    public void releasePatient()
    {
        ...
        System.out.println("Releasing a Patient to a Bed...");
        System.out.println("Getting information...");
        System.out.print("Enter the bed label of the patient: ");
        int bedLabel = consoleIn.nextInt();
        consoleIn.nextLine();  // discard the remainder of the line
